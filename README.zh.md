...
- **控件**:侧边栏底部两个按钮——系统通知(状态圆点:绿=开启 / 红=关闭或阻止 / 黄=点击授权)与静音;样式对齐官方侧边栏操作(宽态整行 / 窄轨 36px 圆形)
...
- **系统通知按钮(侧边栏底部,📢)**:圆点显示权限状态,点击切换/请求授权
  - 未授权(黄点)→ 点击弹出浏览器权限请求,允许后自动开启
  - 已开启(绿点)→ 点击关闭(圆点变红)
- **静音按钮(侧边栏底部,🔔)**:临时静音全部通知(页面内 toast 与系统通知),静音时图标变黄
- **toast(右下角)**:点击跳转对应会话;悬停暂停倒计时;× 手动关闭;同屏最多 3 条;窗口窄于 1480px 时 toast 栈抬升到输入框上方
